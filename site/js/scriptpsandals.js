...
db.collection("shoes").where("cat","==","Sandals").get().then(function(querySnapshot) {
...
if(sizeselected==1){
filterq=firebase.firestore().collection('shoes').where("cat","==","Sandals").where("size","array-contains-any",sizer).where("newpr",">=",low).where("newpr","<=",high);

}
else{
filterq=firebase.firestore().collection('shoes').where("cat","==","Sandals").where("newpr",">=",low).where("newpr","<=",high);
}
