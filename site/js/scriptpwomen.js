...
db.collection("shoes").where("gender","==","Women").get().then(function(querySnapshot) {
...
if(sizeselected==1){
filterq=firebase.firestore().collection('shoes').where("gender","==","Women").where("size","array-contains-any",sizer).where("newpr",">=",low).where("newpr","<=",high);

}
else{
filterq=firebase.firestore().collection('shoes').where("gender","==","Women").where("newpr",">=",low).where("newpr","<=",high);
}
