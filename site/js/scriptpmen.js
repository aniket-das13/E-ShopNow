...
db.collection("shoes").where("gender","==","Men").get().then(function(querySnapshot) {
...
if(sizeselected==1){
filterq=firebase.firestore().collection('shoes').where("gender","==","Men").where("size","array-contains-any",sizer).where("newpr",">=",low).where("newpr","<=",high);

}
else{
filterq=firebase.firestore().collection('shoes').where("gender","==","Men").where("newpr",">=",low).where("newpr","<=",high);
}
