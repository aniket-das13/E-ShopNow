...
db.collection("shoes").where("cat","==","Flip Flops").get().then(function(querySnapshot) {
...
if(sizeselected==1){
filterq=firebase.firestore().collection('shoes').where("cat","==","Flip Flops").where("size","array-contains-any",sizer).where("newpr",">=",low).where("newpr","<=",high);
  
}
else{
filterq=firebase.firestore().collection('shoes').where("cat","==","Flip Flops").where("newpr",">=",low).where("newpr","<=",high);
}
