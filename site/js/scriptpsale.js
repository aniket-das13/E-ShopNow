...
db.collection("shoes").where("disc","in",[10,20,30,40]).get().then(function(querySnapshot) {
...
if(sizeselected==1){
filterq=firebase.firestore().collection('shoes').where("size","array-contains-any",sizer).where("newpr",">=",low).where("newpr","<=",high);

}
else{
filterq=firebase.firestore().collection('shoes').where("disc","in",[10,20,30,40]).where("newpr",">=",low).where("newpr","<=",high);
}
