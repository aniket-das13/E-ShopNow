...
db.collection("shoes").where("cat","==","Shoes").get().then(function(querySnapshot) {
...
if(sizeselected==1){
filterq=firebase.firestore().collection('shoes').where("cat","==","Shoes").where("size","array-contains-any",sizer).where("newpr",">=",low).where("newpr","<=",high);

}
else{
filterq=firebase.firestore().collection('shoes').where("cat","==","Shoes").where("newpr",">=",low).where("newpr","<=",high);
}
